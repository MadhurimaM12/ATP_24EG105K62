...
commonApp.post("/users", upload.single("profileImageUrl"), async (req, res) => {
  ...
  try {
    ...
    // Temporarily disabled - add your Cloudinary API keys to .env
    // if (req.file) {
    ...
    // newUser.profileImageUrl = cloudinaryResult?.secure_url || newUser.profileImageUrl || "https://example.com/default-avatar.png";
...
commonApp.put("/password", verifyToken("USER", "AUTHOR", "ADMIN"), async (req, res) => {
  ...
  const isMatched = await compare(currentPassword, req.user.password);
  if (!isMatched) {
    return res.status(400).json({ message: "Current password is incorrect" });
  }
  // hash new password
  const hashedNewPassword = await hash(req.body.newPassword, 12);
